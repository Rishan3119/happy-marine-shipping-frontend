...
export default function UpdaetViewShip() {
  ...
  // fetch Category  data
  // Function to merge ship amenities with available amenities
const mergeAmenities = (shipAmenities, allAmenities) => {
  const merged = allAmenities.reduce((acc, amenity) => {
    const found = shipAmenities.find((item) => item.name === amenity.amenities);
...
useEffect(() => {
  async function fetchShipData() {
    try {
      const res1 = await axios.get(`${config.base_url}/api/HappyMarineShipping/single/${id}`);
      if (res1.data.status === 200) {
        const shipAmenities = res1.data.data.amenities;
        ...
        if (res2.data.status === 200) {
          setAmenities(res2.data.data);
          mergeAmenities(shipAmenities, res2.data.data); // Merge ship data with input fields
        }
      }
    } catch (error) {
      console.log(error);
    ...
    async function fetchdata() {
      try {
        ...
        if (res.data.status === 200) {
          console.log(res);
          setAmenity(res.data.data.value);
        } else {
          console.log(res);
        }
      } catch (err) {
