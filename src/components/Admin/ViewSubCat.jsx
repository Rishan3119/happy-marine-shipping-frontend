  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `${config.base_url}/api/HappyMarineShipping/deleteSubCategory/${id}`,
        {
          headers: {
    ...
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <div className="flex flex-1 ">
        ...
        >
          <ul className="mt-10 flex flex-col gap-3 ">
            ...
            <li
              className={`px-3 py-2 text-lg text-[#00c292] ${
          ...
          <div className="bg-white  shadow-sm text-xl w-full p-5">
            <div className="flex flex-wrap justify-between items-center">
              ...
              <div className="flex text-sm gap-2">
                <p>Home</p> <span>/</span>
                <p className="text-[#00c292]">View Sub Category</p>
              </div>
            </div>
          </div>
      ...
      <tbody>
        {allSubCategory.length === 0 ? (
          <tr>
            <td colSpan="4" className="px-6 text-red-500 text-center py-4">
          ...
          allSubCategory.map((itm, index) => (
            <tr key={itm.id} className="border-b border-gray-200">
              ...
              <td className="px-6 text-left py-4">{itm.sub_category_name}</td>
              <td className="px-6 text-left py-4">
                {allCategory.find((cat) => cat.id === itm.category)
              ...
              <td className="px-6 text-right">
                <div className="flex gap-4 sm:gap-8 items-center justify-end">
                  <button
                    ...
                    onClick={()=>navigate(`/admin/updateViewSubCat/${itm.id}`)}
                  >
