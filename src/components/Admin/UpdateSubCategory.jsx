...
export default function UpdateSubCategory() {
    ...
    async function fetchdata() {
      try {
        const res1 = await axios.get(
          `${config.base_url}/api/HappyMarineShipping/singleSubCat/${id}`,
          {
            headers: {
        ...
        if (res1.data.status === 200) {
          ...
          setCategoryTitle(res1.data.data.sub_category_name)
          setCategory(res1.data.data.category)
        } else {
          console.log("error");
        }
  ...
  const handleSubmit = async(e)=>{
   ...
   navigate('/admin/viewSubCategory')
    ...
    try {
        const response = await axios.put(`${config.base_url}/api/HappyMarineShipping/updateSubCategory/${id}`,data,{
          headers:{
            'Content-Type':'multipart/form-data',
          }
        });
        ...
        <div className="flex-1 py-4">
          ...
          <div className="flex justify-between items-center">
              ...
              <div className="flex text-sm gap-2">
               <p>Home</p> <span>/</span>
               <p className="text-[#00c292]">Edit Sub Category</p>
              </div>
            </div>
          </div>
