  ...
  const [isScrolled, setIsScrolled] = useState(false);
      ...
      const handlePageChange = (page) => {
        if (page >= 1 && page <= totalPages) {
          navigate(`/Fishing?page=${page}`); // Update URL first
          setCurrentPage(page);
          setRefPage(page);
        }
      };
  ...
  return (
    <div className="">
      ...
      <section className="py-10 bg-gradient-to-r from-[#264861] to-[#326e99] ">
        <h1 className="text-white text-center text-3xl font-bold">FISHING</h1>
        ...
        <div className="w-[85%] Lg:w-[90%] border border-white rounded py-10 px-5  m-auto mt-10 ">
          ...
          <div className="flex justify-center gap-8 lg:flex-wrap lg:gap-5 items-center">
            <button
               onClick={() => navigate(`/Fishing?page=${currentPage}`)}
              className={`${
                Type === "grid"
            ...
            <button
               onClick={() => navigate(`/Fishing/list?page=${currentPage}`)}
              className={`${
                Type === "list"
                  ? "bg-[#d1a460] text-white"
                  : "bg-white text-[#d1a460]"
