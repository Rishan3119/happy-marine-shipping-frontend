          ...
          <div className="mt-5 flex lg:flex-wrap gap-10 items-start">
            ...
            <div>
              ...
              <li
                    ...
                    className="border-b border-white py-1 text-[18px] text-white font-bold cursor-pointer hover:text-white"
                  >
                    All Ships
                  </li>
                {[
                  ...
                  <li
                    key={idx}
                    className="border-b border-white py-1 text-[16px] cursor-pointer hover:text-white"
                  >
                    {type}
                  </li>
                ))}
...
>
  {paginatedGroups.length === 0 ? (
    <div className="col-span-full text-center">
      <p className="text-xl mt-16 text-red-600">No Ships Available...</p>
    </div>
  ) : (
  ...
  {paginatedGroups.length > 0 &&
    ...
    Array.from({ length: 9 - paginatedGroups.length }).map((_, index) => (
      <div
...
<div className="  flex justify-center items-center gap-3">
  <button
    ...
    className={`px-4 py-2 bg-[#123d5f] text-white rounded-sm ${
      currentPage === 1 ? "opacity-50 cursor-not-allowed" : "hover:bg-[#172f41ed]"
    }`}
  ...
  <button
    ...
    className={`px-4 py-2 bg-[#0d2c45] text-white rounded-sm ${
      currentPage === totalPages ? "opacity-50 cursor-not-allowed" : "hover:bg-[#172f41ed]"
    }`}
            ...
            {Type === "list" &&(
                ...
                <table className="2xl:w-[900px] w-full  border border-white">
                  <thead className="bg-[#d1a460] ">
                    <tr>
                      <th className="text-white   border-r border-white w-[200px] py-4">Ship Title</th>
                      <th className="text-white   border-r border-white w-[300px] py-4">Ship Image</th>
                      <th className="text-white   border-r border-white  w-[200px] py-4">Vessel Type</th>
                      <th className="text-white   border-r border-white w-[200px] py-4">Flag</th>
                      <th className="text-white   border-r border-white w-[200px] py-4">Short Description</th>
                      <th className="text-white   border-r border-white w-[140px] py-4">Built Year</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedGroups.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="text-center py-4">No Ships Available</td>
                      </tr>
                    ) : (
                      paginatedGroups.map((ship, index) => (
                        <tr key={index} className="border-b">
                          <td className="px-4 py-2 text-[#d1a460]  text-center  border-r border-white">{ship.title}</td>
                          <td className="px-4 py-2 text-white border-r border-white">
                            <img src={ship.image} alt={ship.title}  text-center className="w-full h-30 object-cover rounded-md" />
                          </td>
                          <td className="px-4 py-2 text-white border-r  text-center border-white">{ship.vessel_type}</td>
                          <td className="px-4 py-2 text-white border-r  text-center border-white">{ship.flag}</td>
                          <td className="px-4 py-2 text-white border-r text-center border-white">{ship.short_description}</td>
                          <td className="px-4 py-2 text-white border-r text-center border-white">{ship.year_built}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
