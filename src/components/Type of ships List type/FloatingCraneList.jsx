...
export default function FloatingCraneList() {
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= totalPages) {
      navigate(`/Floating-Crane/list?page=${page}`); // Update URL first
      setCurrentPage(page);
      setRefPage(page);
    }
  };
    ...
    try {
      ...
      if (res1.data.status === 200) {
        ...
        filteredData = filteredData.filter(
          (ship) => ship.vessel_type === "Floating Crane"
        );
  ...
  return (
    <div className="">
      ...
      <section className="py-10 bg-gradient-to-r from-[#264861] to-[#326e99] ">
        <h1 className="text-white text-center text-3xl font-bold">FLOATING CRANE</h1>
        ...
        <div className="w-[85%] Lg:w-[90%] border border-white rounded py-10 px-5  m-auto mt-10 ">
          ...
          <div className="flex justify-center gap-8 lg:flex-wrap lg:gap-5 items-center">
            <button
             onClick={() => navigate(`/Floating-Crane?page=${currentPage}`)}
              className={`${
                Type === "grid"
            ...
            <button
              onClick={() => navigate(`/Floating-Crane/list?page=${currentPage}`)}
              className={`${
                Type === "list"
                  ? "bg-[#d1a460] text-white"
                  : "bg-white text-[#d1a460]"
                    ...
                    {paginatedGroups.length === 0 ? (
                      <div className="col-span-full text-center">
                        <p className="text-xl mt-16 text-red-600">
                          {" "}
                          Floating Crane Ships not Available...
                        </p>
                      </div>
                    ) : (
